      ...
      Program: function Program(path, state) {
        ...
        if (extension === '.ts') {
          (function () {
            ...
            try {
              (0, _babelTraverse.default)(path.node, {
                enter: function enter(program) {
                  var declaration = program.node.declaration;

                  if (declaration && declaration.type === 'ClassDeclaration') {
                    declaration.body.body.filter(function (node) {
        ...
        (path.get('params') || []).slice().forEach(function (param) {
          ...
          (param.node.decorators || []).slice().forEach(function (decorator) {
            ...
            if (functionName === className) {
              ...
              var callExpression = types.callExpression(decorator.expression, [types.Identifier(className), types.Identifier('undefined'), types.NumericLiteral(param.key)]);
            ...
            var expression = types.expressionStatement(resultantDecorator); // TODO the order of insertion
